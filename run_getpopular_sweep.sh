#!/usr/bin/env bash
# Run getpopular workload sweeps and capture metrics into CSV.
...
OUTPUT=${OUTPUT:-results_getpopular.csv}
...
for t in $THREADS_LIST; do
  ...
  cpu_log=$(mktemp /tmp/getpopular_cpu.XXXXXX)
  ...
  run_out=$(./loadgen getpopular --url "$URL" --threads "$t" --duration "$DURATION" --hot-keys "$HOT_KEYS")
